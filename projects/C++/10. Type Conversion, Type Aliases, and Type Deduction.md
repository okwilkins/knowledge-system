...
Distance miledToDestination{3.4};
```
...
- There are three common conventions:
	1. Type aliases end in a `_t` suffix.
		- This convention is often used by the standard library for globally scoped type names like `size_t` and `nullptr_t`.
			- This convention was inherited from C and used to be the most populate when defining your own type aliases but has fallen out of favour in modern C++.
	...
	2. Type aliases that end in a `_type` suffix. This convention is used by some standard library types like `std::string` to the name nested type aliases like `std::string::size_type`. 
		- Many such nested type aliases do no use a suffix at all (e.g. `std::string::iterator)`, so this usage is inconsistent at best.
...
- This actually creates a new type that has all the original properties of the original type but the compiler will throw an error if attempting to mix values of the aliased type and the strong typedef.
...
- If type aliases are needed across multiple files, they can be defined in a header file and included into any code files that needed to use the definition.
...
- They have been largely replaced by type aliases in modern C++.

- Typedefs have a few syntactical issues.
- First it's easy to forget whether the name of the typedef or the name of the type to the alias comes first.
```cpp
typedef Distance double; // incorrect
typedef double Distance; // correct
```
- Second, the syntax for typedefs can get ugly with more complex types.
```cpp
typedef int (*FcnType)(double, char);
using FcnType = int(*)(double, char); // Easier to read
```
...
- One of the primary uses of type aliases is to hide platform specific details.
- On some platforms, an `int` is 2 bytes and on other it is 4 bytes.
```cpp
#ifdef INT_2_BYTES
using int8_t = char;
using int16_t = int;
...
This example can be difficult to know what the meaning of the return value is:
```cpp
int gradeTest();
```
- This can be better documenting:
```cpp
using TestScore = int;
TestScore gradeTest();
```
...
- Type aliases also allows the changing of an underlying type in one place and to then not have to change it everywhere else.
- For example, if `short` was being using to store a student ID number and this needed to be changed to a `long`.
...
- **Type deduction** (also sometimes called **type inference**) is a feature that allows the compiler to deduce the type of an object from the object's initialiser.

```cpp
int main() {
	auto d {5.0}; // deduced as a double
	aut i {1 + 2}; // deduced as an int
	auto x {i}; // deduced as an int
	
	return 0;
}
```

- As a warning, prior to C++17, `auto d {5.0};` would deduce `d` to be of type `std::initializer_list<double>`, rather than `double`.
...
int main() {
	auto a; // Unable to deduce the type
	auto b {}; // Unable to deduce the type
	auto c {foo()}; // Invalid as c cannot have type incomplete type void
}
```
...
int main() {
	const int a {5};
	auto b {a}; // b has type int with const dropped
...
- If the type definition of a string literal is needed to be `std::string` or `std::string_view`, the `s` and `sv` literal suffixes are needed.
...
- As `constexpr` is not part of the type system, it cannot be deduced as part of the type system.
...
int main() {
	...
	const auto c {a}; // c has type const double
	constexpr auto d {a}; // d has type const double
...
- Second, type deduction only works on variables that have initialisers, so if someone has the habit of using type deduction, it can help avoid unintentionally uninitialised variables:
```cpp
int x;
auto y; // the compiler will error out because it can't deduce a type for y
```

- Third, it is guaranteed that there will be no unintended performance-impacting conversions:
```cpp
std::string_view getString();

std::string s1 {getString()}; // bad: expensive conversion from sv to s
...
- A good IDE should be able to show you the deduced type but it's still a bit easier to make type-based mistakes when using type deduction.

```cpp
auto y {5}; // we wanted a double hyere but accidentally provided an int literal
```

- Second, if the type of an initialiser changes, the type of a variable using the type deduction will also change, perhaps unexpectedly.

Best practice:
- Use type deduction for variables when the type of the object doesn't matter.
- Favour an explicit type when requiring a specific type that differs from the type of the initialiser, or when the object is being used in a context where makes the type obvious is useful.
...
- The biggest advantage of return type deduction is that having the compiler deduce the function's return type negates the risk of an mismatched return type, preventing unexpected conversions.
- This can be particularly useful when a function's return type is fragile. 

```cpp
// let the compiler determine the return type of unsigned short + char
...
- This makes sense, a forward declaration doe not have enough information for the compiler to deduce the function's return type.
- This means normal function that return `auto`  are typically only callable from within the file in which they are defined.

2.  When using type deduction objects, the initialiser is always present as part of the same statement. It's usually not overly burdensome to determine what type will be deduced. With type deduction for functions, that is not the case.

- Unlike type deduction for objects, there isn't as much consensus on best practices for function return type deduction.
- However, prefer explicit return types over return type deduction, except in cases where the return type is unimportant, difficult to express or fragile.

### Trailing return type syntax

- The `auto` keyword can also be used to declare function using a **trailing return syntax**.
...
Why would you wants to use these:
...
2. The trailing return type syntax can be used to align the names of functions, which makes consecutive function declarations easier to read:
...
3. If a function has return types that must be deduced based on the type of the function parameters, a normal return type will not suffice. This is because the compiler has not yet seen the parameters at that point.
...
4. The trailing return syntax is also required for some features, like lambdas.

### Type deduction can’t be used for function parameter types

- Many new programmers that learn about type deduction try something like this:
```cpp
#include <iostream>
...
- In C++20, the `auto` keyword was extended so that the above program will compile and function correctly.
- However, `auto` is not invoking type deduction in this case.
- It is triggering a different feature called `function templates`, that was designed to actually handle such cases.