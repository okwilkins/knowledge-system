...
- **Pointer arithmetic** is a feature that allows certain integer arithmetic operators (addition, subtraction, increment or decrement) to be applied to a pointer to produce a new memory address.
...
- Technically, the above is undefined behaviour.
- Accoring to the C++ standard, pointer arithmetic is only defined behaviour when the pointer and the result within the same array (or one-past-the-end).
- However, modern C++ implementations generally do not enforce this and will typically not disallow the use of pointer arithmetic outside of arrays.
...
- When first learning about array subscripting, it's natural to assume that the index represents a fixed element within the array.
	- Index 0 is the first element, index 1 is always the second element, etc...
- This is an illusion.
- Array indices are actually relative positions.
	- The indices just appear fixed because almost always the start (element 0) of the array  is indexed from.
- Remember, given some pointer `ptr`, both `*(ptr + 1)` and `ptr[1]` return the next object in memory.
	- This is based on the type being pointed to.
- Next is a relative term, not an absolute one.
	- Thus, if `ptr` is pointing to element 0, then both `*(ptr + 1)` and `ptr[1]` will return element 1.
	- But if `ptr` is being pointed to element 3 instead, then both `*(ptr + 1)` and `ptr[1]` will return element 4.
...
int main() {
	...
	// arr decays into a pointer to element 0
	const int *ptr{arr};

	// Prove that pointing to element 0
	std::cout << *ptr << ptr[0] << '\n';
	...
	// Prove that pointing at element 3
	std::cout << *ptr << ptr[0] << '\n';
	// Promve that ptr[1] is element 4
	std::cout << *(ptr + 1) << ptr[1] << '\n';
...
int main() {
	...
	for (auto e : arr) {
		// Deference the loop variable to get the current element
		std::cout << e << ' ';	
	}
...
- The documentation for range-based for loops, they are typically implemented like this:
...
- It should now be obvious that C-style string objects are just C-style arrays whose element type is `char` or `const char`.
- C-style string literals have fallen out of favour in modern C++ because they are hard to use and dangerous.
...
// An array of 7 char, indices 0 through 6
const char str2[]{"string"};
// An array of 6 char, indices 0 through 5
constexpr char str3[]{"hello"};
```

- Remember that an extra character is needed for the implicit null terminator.

### C-style strings will decay

- Because C-style strings are C-style arrays, they will decay.
- C-style string literals will decay into a `const char*` and C-style string arrays decay into either a `const char*` or `char*` depending on whether the array is const.
- The loss of the length information is the reason why C-style string have a null-terminator.
...
- If trying to print a string that does not have a null terminator, the result will be undefined behaviour.

### Inputting C-style strings

- Because C-style strings are fixed-size arrays, when needing input of an unknown length, the solution is to declare an array much larger than is needed.
...
- Prior to C++20, `std::cin >> rolls` would extract as many characters as possible to `rolls`.
- Nothing is stopping the user from entering more than 254 characters (excluding the null terminator).
- If this happens, the user's input will overflow the `rolls` array and undefined behaviour will result.

- In C++20, `operator>>` was changed so that it only works for inputting non-decayed C-style strings.
- This allows `operator>>` to extract only as many characters as the C-style string's length will allow, preventing overflow.
- This means also that `operator>>` cannot be used to input on decayed C-style strings.
...
- Because C-style strings are C-style arrays, `std::size()` (or in C++20 `std::ssize()`) to get the length of the string as an array.
...
- Because C-style strings are the primary string type in C, the C language provides many function for manipulating C-style string.
- These functions have been inherited by C++ as part of the `<cstring>` header.

- Here are a few of the most useful that can be seen in older code:
	- `strlen()` -- returns the length of a C-style array.
...
- With the exemption of `strlen()`, these should generally be avoided. 

### Avoid non-const C-style string objects

- Unless there is a need, avoid non-const C-style string objects in favour of `std::string`.
...
int main() {
	// Case 1: const C-style string initialised with C-style string literal
	const char name[]{"Alex"};
	
	// Case 2: const pointer to C-style string literal
	const char* const colour{"Orange"};
	
	std::cout << name << ' ' << colour << '\n';
	
	return 0;
}
```

- While the above two methods produce the same results, C++ deal with the memory allocation of these slightly differently.
- In case 1, "Alex" is put into (probably read-only) memory somewhere.
	- Then the program allocates memory for a C-style array of length 5 and initialises that memory with the string "Alex".
	- There will be two copies of "Alex".
		- One in global memory somewhere.
		- The other owned by `name`.
	- Since `name` is const (and will never be modified), making a copy is inefficient.
- In case 2, how the compiler handles with this is implementation defined.
	- What usually happens is that the compiler places the string "Orange" into read-only memory somewhere and then initialises the pointer with the address of the string.

### Favour std::string_view for C-style string symbolic constants

 - There is little reason to use C-style string symbolic constants in modern C++.
 - Instead, favour `constexpr std::string_view` objects, which tend to be just as fast (if not faster) and behave more consistently.
...
- Memory is linear (1D), so multidimensional arrays are actually stored as a sequence list of elements.
...
- In C++, when initialising an array, elements are initialised in row-major order.
- When traversing an array, it is most efficient to access elements in the order they are laid out in memory.

### Initialising two-dimensional arrays

- Easiest to use nested braces.
...
- An initialised multidimensional array can omit (only) the leftmost length specification.
...
- When initialising a multidimensional `std::array`, double braces are needed.
- The syntax is verbose and hard to read.
- Because of the way templates get nested, the array dimensions are switched.

- Indexing works the same as a two-dimensional C-style array.

```cpp
std::cout << arr[1][2];
```

- A two dimensional `std::array` can be passed to a function, just like a one-dimensional `std::array`.
...
- If this were to be used, there would need to be a new one for every combination of element types and dimensions used.
...
template <typename T, std::size_t Row, std::size_t Col>
using Array2d = std::arrayK<std::array<T, Col>, Row>;
