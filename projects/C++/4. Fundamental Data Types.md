...
- The "floating point" refers to that the decimal point can "float".
...
- On modern architectures, floats are conventionally implemented using one of the floating-point standards defined in the IEEE 754 standard.
- Because of this, `float` is almost always 4 bytes.
	- Specifically the IEEE 754 singe-precision format.
- A `double` will almost always be 8 bytes
	- Specifically the IEEE 754 double-precision format.

- The format used to implement a `long double` varies by platform.
	- 8-byte IEEE 754 double precision format. 
	- 80-bit (often padded to 12 to 16 bytes) x87 extended-precision format.
		- This is compatible with IEEE 754.
...
- Note that by default that floating point literals default to a `double`.
...
- A `float` has 6 to 9 digits of precision.
	- This means it can exactly represent any number up to 6 significant digits and a number with 7 to 9  digits may or may not be represented exactly, depending on the number.
- A `double` can have between a 15 to 18 digits of precision.
	-  Most double values will have 16 significant digits.
- A `long double` has a min precision of 15, 18 or 33 significant digits.

- It is best practice to favour a `double` over a `float`, unless space is at a premium. It's very easy to have inaccuracies.

IEEE 754 also can support:
- **Inf**`
- **NaN**
- Singed zero

Avoid division by `0.0`, even if your compiler can support it.
...
- `std::cout` will print `0` for `false` and `1` for `true`.
```cpp
#include <iostream>
...
If you want to print `true` and `false` instead of `0` and `1` use `std::boolalpha`.
- It manipulates the way `std::cout` outputs `bool` values.
- To turn it back off again just call `std::boolalpha` again.
...
int main() {
	bool bFalse {0}; // ok: init to false
	bool bTrue  {1}; // ok: init to true
	bool bNo    {2}; // error: narrowing conversions disallowed
}
```

- If you were to do `bool b1 = 4;` it would eval to `true`.


- If you want `std:cin` to work with "true" and "false", you must use `std::cin >> std::boolalpha;`. Otherwise it will silently fail and convert a bool to false. 


## 4.10 Introduction to if statements

Not much to write here on the basics!
...
- This will print `hi` as `x` has a non-zero value that gets converted to a boolean `true`.

## 4.11 Chars

- The char type is an integral type, meaning the underlying value is stored as an integer.
- **ASCII** stands for American Standard Code for Information Interchange.
- It defines a way of representing English characters as numbers between 0 and 127.
	- This is called ASCII code or **code print**.
...
You and init chars with integers as well but this should be avoided if possible:
```cpp
char ch2{97};
```

- By default a char may be signed or unsigned
- If using chars to hold ASCII chars, you don't need to specify a sign, as both signed and unsigned chars can hold values between 0 and 127.


- **Escape sequences** start with a `\` and represent a single char.
...
- Avoid multicharacter literals, like `'56'`. These are not supported in the C++ standard and are compile dependant.

- For other languages, Unicode can be used.
- A single Unicode point needs 32-bits, called UTF-32.
	- They can also be encoded in UTF-16 and UTF-8 too.

- `char16_t` and `char32_t` were added to C++11 to provide explicit support for 16-bit and 32-bit Unicode characters.
- These char types have the same size as `std::uint_least16_t` and `std::uint_least32_t` respectively.
- `char8_t` was added in C++20 to provide support for 8-bit Unicode. It uses the same representation as `unsigned char`.

- You won't need to use `char8_t`, `char16_t` or `char32_t`, unless you're planning to make your program Unicode compatible.
- `wchar_t` should be avoided in almost all cases as its size is implementation-defined.
...
- We are using a `double` for input but parsed in an `int`.
- In most cases C++ will allow us to convert values from one fundamental type to another.
- When a compiler does type conversion on our behalf without us explicitly asking, we call this **implicit type conversion**.

- Type conversion does not modify the value (or object), instead, the conversion process uses that data as input and produces the converted result.
- Some advanced type conversions (`const_cast` and `reinterpret_cast`) do not return temporary objects but instead reinterpret the type of an existing value or object.


- `static_cast` can be used for **explicit type conversion**.
- To perform this you would do:
```cpp
static_cast<new_type>(expression)
```

- As of C++20, if converting from a signed to an unsigned, the value will be modulo wrapped.
- Prior to C++20, it was implementation specific.