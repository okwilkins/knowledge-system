...
- The range-based for statement has syntax that looks like this:
...
- When a range-based for loop is encountered, the loops will iterate through each element  in `array_object`.
- For each iteration, the value of the current array element will be assigned to the variable declared in `element_declaration`.
...
- Because `num` is assigned the value of the array element, this does mean that array element is copied.
	- This can be expensive for some types.

### Range-based for loops and type deduction using the auto keyword

- Because `element_declaration` should have the same type as the array elements, this is an ideal case in which to use the `auto` keyword.
...
- It's best practice to this.

- Another advantage is that if the element type is ever updated, `auto` will automatically deduce the updated element type.
...
- Normally `auto` is for cheap-to-copy types, `auto&` when wanting to modify the elements and `const auto&` for expensive-to-copy types.
- With range-based for loops, many developers believe it is preferable to always use `const auto&` because it is more future-proof.

### Range-based for loops and other standard container types

- Range-based loops work with a wide variety of array types, including (non-decayed) C-style arrays, `std::array`, `std::vector`, linked list, trees and maps.
...
- This is because a range-based for-loop needs to know the length of the array to know when traversal is complete.
	- Decayed C-style arrays do no contain this information.

### Getting the index of the current element

- Range-based for loops do not provide a direct way to get the array index of the current element.
	- This is because many of the structures that a range-based for loop can iterate over (such as `std::list`) do not support indices.
- However, range-based for loops always iterate in a forwards direction and don't skip elements.
	- This means a counter can be declared and incremented.
	- If going for this, then consider if it's better off using a normal for loop.

### Range-based for loops in reverse

- Range-based for loops only iterate in forwards order.
- In C++20, it's possible to use `std::views::reverse` in the Ranges library to create a reverse view of the elements that can be traversed.
...
- One of the bigger documentation problems with arrays is that the integer indices do not provide any information to the programmer about the meaning of the index.
...
- This means they can be used as array indices to help document the meaning of the index.
...
- Because enumerators are implicitly costexpr, conversion of an enumerator to an unsigned integral type is not considered a narrowing conversion, thus avoiding signed/unsigned indexing problems.
...
- The underlying type of an unscoped enumeration is implementation defined.
	- Thus, it could be either a signed or unsigned integral type.
	- Because enumerators are implicitly constexpr, as long as indexing with unscoped enumerators, there won't be any sign conversion issues.
- However, if defining a non-constexpr variable of the enumeration type and then try to index the `std::vector` with that, there may be sign conversion warnings on any platform that defaults unscoped enumerations to a signed type.
...
int main() {
	std::vector testScores{78, 94, 66, 66, 14};
	// Non-const expr
	Students::Names name{Students::stan};

	// May trigger a sign conversion warning if Student::Names defaults to a signed underlying type
	testScores[name] = 76;
}
```

- In this particular case, `name` could be made constexpr.
	- However, that won't work when the initialiser isn't a constant expression.
- An alternate option is to explicitly specify the underlying type of the enumeration to a unsigned int.
...
- A **count enumerator** is a value that represents the count of previously defined enumerators.
...
- More often, an array using initialiser list of values is used.
	- This is with the indent of indexing that array with enumerators.
- In such cases, it can be useful to assert that the size of the container equals the count enumerator.
...
- If the array is constexpr, then use `static_assert` instead.
	- `std::vector` doesn't support constexpr but `std::array` and C-style arrays do.

### Arrays and enum classes

- Enum classes don't have an implicit conversion to integral types.
- Because of this, `static_cast` can be used to index into an array.
- This significantly clutters up the code however.
...
- Most array types have a significant limitation:
	- The length of the array must be known at compile time and cannot change.
- Such arrays are called **fixed-size arrays** or fixed-length arrays.
- Both `std::array` and `C-style` arrays are fixed-sized arrays.

- `std::vector` is a dynamic array.
- A **dynamic array** (also called a **re-sizeable array**) is an array whose size can be changed after instantiation.
...
- When resizing, the new elements are zero-initialised.
...
- Informally, the reallocation process goes something like this:
	- The `std::vector` acquires new memory with capacity for the desired number of elements.
		- These elements are value-initialised.
	- The elements in the old memory are copied (or moved, if possible) into the new memory.
		- The old memory is returned to the system.
	- The capacity and length of the `std::vector` has been resized.
		- But internally, the memory (and all of the elements) have actually been replaced.

- Because reallocation typically requires every element in the array to be copied, reallocation is an expensive process.

### Vector indexing is based on length, not capacity

- The `operator[]` and `at()` member function is based on the vector's length, not capacity.
- Even though the elements with indices between the length of 3 (inclusive) and capacity of 5 (exclusive) may exist, their indices are considered to be out of bounds.
...
- Reallocating a vector just to reclaim the memory from a small number of elements that are no longer needed is a poor choice.
	- However, in the case where there is a vector with a large number of elements that are not needed, the memory waste could be substantive.
- `shrink_to_fit()` requests that the vector shrink its capacity to match its length.
	- This request is non-binding, meaning the implementation is free to ignore it.
	- Depending on what the implementation thinks is best, it may decide to fulfil the request or partially fulfil it or completely ignore it. 
...
## 16.11 — std::vector and stack behaviour
...
- In some languages, a stack is implemented as its own discrete container type, separate from other containers.
	- However, this can be quite limiting.
- Consider the case where all values in a stack need to be printed, without modifying the stack.
	- A pure stack interface does not provide a direct method to do this.

- In C++, stack-like operations were instead added (as member functions) to the existing standard library container classes.
	- These support efficient insertion and removal of elements at one end:
		- `std::vector`
		- `std::deque`
		- `std::list`
...
- `push_back()` and `emplace_back()` will increment the length of a `std::vector` and will cause a reallocation to occur, if the capacity is not sufficient to insert a value.
...
- As a result, the prior program will have slightly different output, depending on what compiler is used.

### Resizing a vector doesn’t work with stack behaviour

- In the above example, 3 allocations could have been prevented, if the vector was manually resized.
- However, simply doing this won't work:
...
- The same goes for the `resize()` member function.
- There are problems here, when using a vector as a stack.
...
- Both push an element onto the stack.
	- If the object to be pushed already exists, `push_back()` should be preferred.
- However, in cases where creating a temporary object, for the purpose of pushing it onto the vector, `emplace_back()` can be more efficient.
...
int main() {
	...
	stack.push_back({"a", 2}); // creates a temp object and copies it into the vector
...
- `std::vector` has an interesting trick up its sleeves.
- There is a special implementation for `std::vector<bool>` that may be more space efficient for Boolean values by similarly compacting 8 Boolean values into a byte.
	- Similar to `std::bitset` covered in lesson O.1.
...
- First, `std::vector<bool>` has a fairly high amount of overhead.
	- `sizeof(std::vector<bool>)` is 40 bytes.
	- So this is not saving memory, unless allocating more boolean values than the overhead of the architecture.
- Second, the performance of `std::vector<bool>` is highly dependant upon the implementation.
	- This is because implementations aren't even required to do optimisation, let alone do it well.
- Third and most importantly, `std::vector<bool>` is not a vector (it is not required to be contiguous in memory), nor does it hold `bool` values (it holds collections of bits), nor does it meet C++'s definition of a container.
- Although `std::vector<bool>` behaves like a vector, in most cases it is not full compatible with the rest of the standard library.

- This works when `T` is any type except `bool`.
...
- The modern consensus is that `std::vector<bool>` should be generally avoided.
	- The performance gains are unlikely to be worth the incompatibility headaches due to it not being a proper container.
- Unfortunately, this optimising of `std::vector<bool>` is enabled by default and there is no way to disable it in favour of an non-optimised version that is a actually a container.

- Instead, try these:
	- Constexpr `std::bitset` when the number of bits needed is known at compile time, there isn't a need to have more than a moderate number of Boolean values to store and the limited set of operators and member functions meet the requirements of the program.
	- Prefer `std::vector<char>` when needing a resizable container that Boolean values and space-savings isn't a necessity.
		- This type behaves normally.
	- Favour a 3rd party implementation of a dynamic bitset (such as `boost:dynamic_bitset`) when needing a dynamic bitset to do bit operations on.
