...
- In case 1, the `IntPair p` variable is being instantiated and then passing `p` to the function `print()`.
- However, `p` is only used once and function `print()` will accept rvalues, so there is really no reason to define a variable here.
- This can be done by passing a temporary object instead of a named variable.
	- A **temporary object** (sometimes called an **anonymous object** or **unnamed object**) is an object that has no name and exists only for the duration of a single expression.
...
int main() {
	...
	// Case 3: Implicitly convert {7, 8} to a temporary Intpair and pass to to function
	print({7, 8});
...
- In cases where it is needed to convert a value from one type to another but narrowing conversions aren't involved, often it is need to have the option to either use `static_cast` or explicit instantiation of a temporary object.
...
int main() {
	...
	// static_cast returns a temporary int direct-initialised with value c
	std::cout << static_cast<int>(c) << '\n';
	
	// Explicitly creates a temporary int list-initialised with value c
	std::cout << int{c} << '\n';
...
int main() {
	...
	// Compile error: a std::string_view won't implicitly convert to a std::string
	printString(sv);

	// Case 1: static_cast returns a temporary std::string direc-initialised with sv
	printString(static_cast<std::string>(sv));
	// Case 2: explicitly creates a temporary std::string list-initialised with sv
	printString(std::string{sv});
	// Case 3: C-style cast returns temporary std::string direct-initialised with sv (avoid this one!)
	printString(std::string(sv));
...
- Best practice: as a rule of thumb, prefer `static_cast` when converting to a fundamental type and a list-initialised temporary when converting to a class type.
- Prefer `static_cast` when creating a temporary object when any of the following are true:
	- Need to perform a narrowing conversion.
	- Need to make it really obvious that a type conversion will result in some different behaviour.
		- For example a `char` to an `int`.
- Prefer creating a new object (using list initialisation) to create a temporary object when any of the following are true:
	- Wanting to use list-initialisation.
		- For example for the protection against narrowing conversions or because the list constructor needs to be invoked.
		- Provide additional arguments to a constructor to facilitate the conversion.
...
- What constructor is `fCopy` using?
	- The answer: the copy constructor.

### The copy constructor

- A **copy constructor** is a constructor that is used to initialise an object with an existing object of the same type.
	- After the copy constructor executes, the newly created object should be a copy of the object passed in as the initialiser.

### An implicit copy constructor

- If a copy constructor is not provided to a class, C++ will create a public **implicit copy constructor** automatically.
- By default, the implicit copy constructor will do member-wise initialisation.
	- This means each member will be initialised using the corresponding member of the class passing in as the initialiser.
...
public:
	...
	// Copy constructor
	Fraction(const Fraction &fraction) : m_numerator{fraction.numerator},
...
- Unlike the implicit default constructor, which does nothing (and thus is rarely wanted), the member-wise initialisation performed by the copy constructor is usually exactly what is wanted.

### The copy constructor’s parameter must be a reference

- It is a requirement that the parameter of a copy constructor be an lvalue reference or const lvalue reference.
	- Because the copy constructor should not be modifying the parameter, using a const lvalue reference is preferred.

### Pass by value and the copy constructor

- When an object is passed by value, the argument is copied into the parameter.
	- When the argument and parameter are the same class type, the copy is made by implicitly invoking the copy constructor.
...
public:
	...
	Fraction(const Fraction &f) : m_numerator{numerator},
		m_denominator{denominator} {
		std::cout << "Copy constructor called\n";		
	}
...
int main() {
	...
	// f is copied into the function parameter using the copy constructor
	printFraction(f);
...
- When returning by value, it creates a temporary object (holding a copy of the return value) that is passed back to the caller.
	- When the return type and return value are the same class type, the temporary object is initialised by implicitly invoking the copy constructor.
...
- There will be three copies made:
	- `generateFraction` returns by value, so invoked the copy constructor.
	- When list initialising `f2`, as the types going in and `f2` are the same, the copy constructor is called.
	- `printFunction` is accepting a `Fraction` by value, so the copy constructor is invoked there too.
...
- If a class has no copy constructor, the compiler will implicitly generate one.
	- If it's preferential, the compiler can be explicitly requested to create a default copy constructor using the `default=` syntax.
...
public:
	...
	// Explicitly request a default copy constructor
...
## 14.15 — Class initialisation and copy elision

### Copy elision

- Since the compiler is free to rewrite statements to optimise them, one might wonder if the compiler can optimise away an unnecessary copy and treat `Something s{Something{5}};` as if it were written as `Something s{5};`.
- The answer is yes and the process of doing so is called copy elision.
	- **Copy elision** is a compiler optimisation technique that allows the compiler to remove unnecessary copying of objects.
	- When the compiler optimises away a call to the copy constructor, it is said that the constructor has been **elided**.
- Unlike other types of optimisation, copy elision is exempt from the "as-if" rule.
	- That is, copy elision is allowed to elide the copy constructor, even if the copy constructor has side effects.
	- This is why the copy constructor should have no side effects other than copying.

### Copy elision in pass by value and return by value

- The copy constructor is normally called when an argument of the same type as the parameter is passed by value or return by value is used.
- However, in certain cases, these copies may be elided.
...
- In C++14 or older, with copy elision disabled, the above program would called the copy constructor 4 times.
	- However due to copy elision, it's likely that the compiler will elide most or all of these copy constructor calls.
- It is not important to memorise when the compiler does/doesn't do copy elision.
	- Just know that it is an optimisation that the compiler will perform if able.

### Mandatory copy elision in C++17

- Prior to C++17, copy elision was strictly an optional optimisation that compilers could make.
	- In C++17, copy elision became mandatory in some cases.
	- In these cases, copy elision will be performed automatically, even if telling the compiler not to perform copy elision.
...
- `printFoo` has a `Foo` parameter but an argument of type `int` is being passed in.
	- Because these types do not match, the compiler will try to implicitly convert int value `5` into a `Foo` object, so the function can be called.
- Unlike the case of having a parameter and argument types being both fundamental types, where it can be converted using the built-in numeric promotion/conversion rules.
	- In this case, one of the types is a program-defined type.
...
- Instead, the compiler will look to see if there is a function defined that it can use to perform such a conversion.
	- Such a function is called a **user-defined conversion**.

### Converting constructors

 - In the above example, the compiler will find a function that lets it convert int value `5` into a `Foo` object.
	 - That function is the `Foo(int)` constructor.
 - Up to this point, typically constructors have been used to explicitly construct objects.

```cpp
Foo x{5}; // Explicitly convert int value 5 to a Foo
```

- Think about what this does: an `int` of value `5` is being provided and getting a `Foo` object in return.

- In the context of a function call, the same problem is trying to be solved.

```cpp
print(5); // Implicitly convert int value 5 into a Foo
```

- An `int` of value `5` is being provided and a `Foo` object is wanted to be returned.
	- The `Foo(int)` constructor was designed for exactly that.
- So in this case, when `printFoo(5)` is called, parameter `f` is copy initialised using the `Foo(int)` constructor, with `5` as an argument.
...
int main() {
	printEmployee("Joe"); // supplying a string literal argument
...
- The reason is simple:
	...
	- First, the C-style string literal has to be converted to a `std::string_view` (using the `std::string_view` converting constructor) and then the `std::string_view` has to be converted into an `Employee`, using the `Employee(std::string_view)` converting constructor.
...
- Although this may have been the caller's intent, it's hard to tell because the caller did not provide any indication that this is what they actually wanted.
	- It's entirely possible that the caller assumed this would print `5` and did not expect the compiler to silently and implicitly convert the `int` value to a `Dollars` object so that it could satisfy this function call.
- While this example is trivial, in a larger and more complex program, it's fairly easy to be surprised by the compiler performing some implicit conversion that wasn't expected.

### The explicit keyword

- To address such issues, the **explicit** keyword can be used to tell the compiler that a constructor should not be used as a converting constructor.
- Making a constructor `explicit` has two notable consequences:
	- An explicit constructor cannot be used to do copy initialisation or copy list initialisation.
	- An explicit constructor cannot be used to do implicit conversions (since this uses copy initialisation or copy list initialisation).
...
### Explicit constructors can be used for direct and direct list initialisation

- An explicit constructor can still be used for direct and direct list initialisation:

```cpp
// Assume Dollars(int) i8s explicit
int main() {
	Dollars d1(5); // ok
	Dollars d2{5}; // ok
}
...
- Note that `static_cast` returns an object that is direct-initialised, so it will consider explicit constructors while performing the conversion.
...
- When returning a value from a function, if that value does not match the return type of the function, an implicit conversion will occur.
	- Just like with pass by value, such conversions cannot use explicit constructors.
...
class Foo {
public:
	explicit Foo() {} // note: explicit for the sake of example
...
Foo getFoo() {
	// explicit Foo() cases
	return Foo{} // ok
	return {}; // error: can't implicitly convert initialiser list to Foo
	
	// explicit Foo(int) cases
	return 5; // error: can't be implicitly convert int to Foo
	return Foo{5}; // ok
	return {5}; // error: can't implicitly convert initialiser list to Foo
}
...
- The modern best practice to make any constructor that will accept a single argument `explicit` by default.
	- This includes constructors with multiple parameters where most or all of them have default values.
	- This will disallow the compiler form using that constructor for implicit conversions.
	-  If implicit conversion is required, only non-explicit constructors will be considered.
	- If no non-explicit constructor can be found to perform the conversion, the compiler will error.
- If such a conversion is actually desired in a particular case, it is trivial to convert the implicit conversion into an explicit definition using direct list initialisation.
...
- However, if preferred, the above can be marked as explicit to prevent implicit conversions with empty and multiple-argument lists.
...
- There are some occasions when it does make sense to make a single-argument constructor non-explicit:
	...
	- For example, the `std::string_view` constructor that accepts a C-style string argument is not explicit, because there is unlikely to be a case when it wouldn't be ok with a C-style string being treated as a  `std::string_view` instead.
...
int main() {
	constexpr Pair p{5, 6}; // compile error: p is not literaly type
	return 0;	
}
```
...
- When compiling the program, there is a compiler error about `Pair` not being a "literal type"?
- In C++, a **literal type** is any type for which it might be possible to create an object within a constant expression.
	...
	- In the non-aggregate, `Pair` does not qualify.

- The definition of a literal type is complex.
...
- When a class object is instantiated, the compiler will call the constructor function to initialise the object.
	- The constructor function in the `Pair` class is not constexpr, so it can't be invoked at compile-time.
	- Therefore, `Pair` objects cannot be constexpr.

- To fix this: just make the constructor `constexpr` as well:
...
constexpr int init() {
	Pair p{5, 6}; // requires constructor to be constexpr when evaled at compile-time 
	return p.greater(); // requires greater() to be constexpr when evaled at compile-time	
}
...
- Normally `constexpr` and `const` aren't seen right next to each other but one case where this does happen is when having a constexpr member function that returns a const reference (or (const) pointer-to-const).
- In the `Pair` example, `getX()` is a constexpr member function that returns a const reference:
...
- A member function that returns a const pointer to const instead might look something like this:
