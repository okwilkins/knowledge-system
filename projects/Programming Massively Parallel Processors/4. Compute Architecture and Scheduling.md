...
- With Hopper architecture, CUDA introduced an optional hierarchy level called **thread block cluster**.
...
- In general, the higher the compute capability level, the more the resources available in each SM.
	- The compute capability of GPUs also tends to increase from generation to generation.
		- The A100 GPU has a compute capability of 8.0 and the H100 GPU 9.0.
- In CUDA C++, there is a built-in mechanism for host code to query the properties of the devices available in the system.
...
- While it may not be obvious, modern PC systems often has two or more CUDA-capable devices.
	- This is because of the integrated graphics.
- This would be a reason for the host code to iterate through all the available devices, query their resources and capabilities and choose the ones that have enough resources to execute the application to a good enough level.

```cpp
cudaDeviceProp devProp;
for (unsigned int i = 0; i < devCount; ++i) {
	cudaGetDeviceProperties(&devProp, i);
	// Decide if dvice has sufficitent resources/capabilities	
}
```

- `devProp.maxThreadsPerBlock` gives the maximal number of threads allowed in a block in the queried device.
- `cudaOccupancyMaxPotentialClusterSize()` gives the maximum thread block cluster size.
- `devProp.multiProcessorCount` gives the total number of SMs.
- `devProp.clockRate` gives the clock frequency.
- `devProp.maxThreadsDim[0]` (x-dimension), `[1]` (y-dim) and `[2]` (z-dim) gives the max number of blocks allowed along each dimension of a grid.
	- A typical use of this info is to determine whether a grid can have enough threads to handle the entire data set or if some kind of iterative approach is needed.
- `devProp.regsPerBlock` gives the number of registers available in each SM. 
	- This can be useful in determining whether the kernel can achieve max occupancy on a particular device or if it will be limited by register usage.
- `devProp.warpSize` gives the size of warps. 
