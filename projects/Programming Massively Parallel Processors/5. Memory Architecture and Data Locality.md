...
- There is an intrinsic trade-off in the use of device memories in CUDA.
	- The global memory is large but slow whereas the shred memory is small but fast.
- A common strategy is to partition the data into subsets called tiles so that each tile fits into the shared memory.
- The term tile draws on the analogy that a large wall can be covered by small tiles.
	- I.e. subsets that each can fit into the shared memory.
- An important criterion is that kernel computation on these tiles can be done independently of each other.
	- Note that not all data structures can be partitioned into tiles given an arbitrary kernel function.

- As the size of shared memory is quite small, care must be taken to not exceed the capacity of the shared memory.
- The size of titles is chosen, so that they can fit into the shared memory.

 Tiled Matmul kernel using shared memory:
```cpp
#define TITLE_WIDTH 2
...
__global__ void matMulKernel(float *M, float* N, float* P, int Width) {
	...
	// Loop over the M and N tiles requierd to compute P element
	float Pvalue = 0;
	for (int ph = 0; ph < Width / TILE_WIDTH; ++ph) {
		// Collaborative loading of M and N tiles into shared memory
		Mds[ty][tx] = M[Row*Width + ph*TILE_WIDTH + tx];
		Nds[ty][tx] = N[(ph*TITLE_WIDTH + ty)*Width + Col];
		__syncthreads();
...
- First all threads in the block, which is the same size as the tile size, load in the tile of `M` and `N` into shared memory.
	- This is done by mapping the tile of `M`/`N` into `Mds`/`Nds`.
	- Logic for M:
		- `Row*Width` maps the row within M.
		- `ph*TILE_WIDTH` is the tile offset to get the correct tile column.
		- `tx` is the local thread within the tile that will put the data in `Mds`.
	- Logic for N:
		- `ph*TILE_WIDTH` is the tile offset to get the correct tile row.
		- `ty` is the local thread within the tile that will put the data in `Nds`.
		- So `ph*TITLE_WIDTH + ty` describes the row in N to map in `Nds`.
			- As the data is row-major, multiplying by `Width` ensures that the correct index to get the row is found.
		- Adding the `Col` just gets the correct column for the given row.
- After the threads are all synchronised, each thread calculated a partial dot product.
	- It's partial because the cached section of `M` and `N` will not contain the entire row/col of `M`/`N` respectively.
	- Remember that a thread will be accessing an entire row of `M` and column of `N`.
- The tile will then move onto the next section and the process starts again.
	- This keeps occurring till the borders of M and N have been reached.
	- `ph` here can be though of as the tile index.
		- This is also called the number of **phases**.
- The thread's final sum for the dot product can be then placed into matrix `P`.

- The benefit to the tiled algorithm is substanial.
- For matrix multiplication, the global memory access are reduced by a factor of `TILE_WIDTH`.
	- For example with a tile width of `32`, this increases the compute-to-global-memory-access ratio from 0.25 FLOP/B to 8 FLOP/B.
	- This improvement brings the ratio close to the H100's threshold of 20 FLOP/B.

- Note that **tiling** as an optimisation does not only refer to to placing frequently accessed subsets of data into shared memory.
- It is a general optimisation that refers to placing frequently accessed subsets of data in a memory that is faster than where the data previously resides.
- The global memory being placed into shared memory is an obvious example.
	- However, the tiling was also applied to the `P` matrix.
	- Since the same element of `P` is repeatedly accessed by the same thread, it was placed into a register `Pvalue` and repeatedly accessed.
	- This is called **register tiling**.

- The reason why GPUs rely on shared memory, whereas CPUs rely on L1/3 cache.
	- CPUs can do this as they typically have 1 to 2 threads at a time.
- The GPU SM runs many threads simultaneously to be able to hide latency.
	- This makes GPU cache less reliable, necessitating the use of shard memory for important data that is to be reused.


## 5.5 Boundary Checks

![[./images/boundary_tiling.png]]

- When using tiling on matrices, where the tile size is not divisible into the matrix size, problems can occur.
	- Take the example above that is a 3x3 matrix and the tile size is set to 2.
- This is because some tiles will be trying to access data that does not exist.
	- Either data from the wrong data structure will be accessed or undefined behaviour will occur.

- When a thread is to load an input tile element, it should test whether the input element it is attempting to load is a valid one.

```cpp
// Loop over the M and N tile required to compute P element
float Pvalue = 0;
...
- As seen in chapter 4 that register usage can be a limiting factor for occupancy.
- Shared memory usage can also limit the number of threads assigned to each SM.
	- A H100 can have up to 228KB of shared memory per SM and supports a max of 2048 threads per SM.
	- For all threads to be used, a thread block should not use more than an average of (228 KB) / (2048 threads) = 114B/thread.

- For kernel that need to use a very large amount of shared memory, CUDA provides the possibility to reconfigure the SM to use more resources for the shared memory.
	- This is at the expense of other on-chip memory resources, particularly the cache.
	- L1 cache and shared memory use the same part of memory.
- This allows the programmer to apportion the memory resources according to the bet fit of their application.

- The dynamic adjustment of the shared memory size by adding the `extern` keyword in front of the shared memory declaration and omitting the size of the array in the declaration.
	- The declarations for `Mds` and `Nds` need to be merged into one dynamically allocated arrays
	- `extern __shared__ Mds_Nds[];`
	- They are merged as a a block can only have one dynamic shared memory allocation per block.
- At run time, when calling the kernel, the amount of shared memory dynamically allocated can be configured like so:

```cpp
size_t size = ...;
matMulKernel<<<dimGrid, dimBlock, size>>>(...);
```
